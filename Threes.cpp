...
int shell(int argc, const char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (para.find("--name=") == 0 || para.find("--account=") == 0) {
            ...
            std::shared_ptr<agent> play(new tdplayer(para.substr(para.find("=") + 1)));
            host.register_agent(play);
        } else if (para.find("--evil") == 0) {
            std::shared_ptr<agent> evil(new evilenv(para.substr(para.find("=") + 1)));
            ...
            if (std::regex_match(command, match_move)) {
                ...
                if (move == "?") {
                    ...
                    if (a.type() == action::place::type) {
                        ...
                        tile=move[0]-'0';
                        if(tile<4){
                            host.at(id).play->bag[tile-1]--;
                            host.at(id).play->total_amt++;
                        }
                        else{
                            host.at(id).play->total_amt++;
                            host.at(id).play->bonus_amt++;
                        }
                    }


                    //pass hint to player
                    host.at(id).play->hint=hint;
...
int main(int argc, const char* argv[]) {
	...
	while (!stat.is_finished()) {
		...
		while (true) {
            ...
            play.bonus_amt=evil.bonus_amt;
            play.total_amt=evil.total_amt;
            play.bag[0]=evil.bag[0];
            play.bag[1]=evil.bag[1];
            play.bag[2]=evil.bag[2];
